...
async def _llm_hypotheses_structured(fields: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Ask LLM to produce a structured hypotheses bundle.
    Schema:
    {
      "hypotheses": [
        {"title":"...","rationale":["..."],"predictions":["..."],"uncertainties":["..."],"recommendations":["..."]},
        ...
      ]
    }
    """
    ...
    from server.utils.openai_wrapper import chatgpt_call  # type: ignore
    sys = (
        "You are a senior computational catalysis/materials researcher. Given the structured JSON 'fields'\n"
        "(includes research_question, scientific_context, system, operation, prior results),\n"
        "produce STRICT JSON (no prose outside JSON). Default schema:\n"
        "{\n  \"hypotheses\": [\n    {\n      \"id\": \"H1\",\n      \"statement\": \"short scientific hypothesis\",\n      \"rationale\": [\"bullet\"],\n      \"predicted_outcomes\": [\"expected observables/trends\"],\n      \"uncertainties\": [\"assumptions / unknowns\"],\n      \"recommendations\": [\"DFT task to test/falsify\"]\n    }\n  ]\n}\n"
        "Notes:\n"
        "- If operation relates to NEB/diffusion, include expected barrier ranges (eV) and image counts in recommendations.\n"
        "- Use English only. Keep hypotheses concise and specific to the given system/operation.\n"
    )
    user = "FIELDS:\n" + json.dumps(fields, ensure_ascii=False, indent=2)
    try:
...
def _normalize_structured_bundle(data: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize LLM JSON into stable keys and aggregate recommendations.
    Ensures fields: id, statement, rationale[list], predicted_outcomes[list], uncertainties[list], recommendations[list].
    """
    hs = []
    rec_all: List[str] = []
    for i, h in enumerate((data.get("hypotheses") or []), start=1):
        if not isinstance(h, dict):
            continue
        hid = str(h.get("id") or f"H{i}")
        stmt = h.get("statement") or h.get("title") or ""
        def _as_list(x):
            if isinstance(x, list): return [str(t) for t in x if t is not None]
            if isinstance(x, str) and x.strip(): return [x.strip()]
            return []
        rationale = _as_list(h.get("rationale"))
        predicted = _as_list(h.get("predicted_outcomes") or h.get("predictions"))
        uncertainties = _as_list(h.get("uncertainties"))
        recs = _as_list(h.get("recommendations"))
        rec_all.extend(recs)
        hs.append({
            "id": hid,
            "statement": str(stmt),
    ...
    # de-dup recommendations preserving order
    seen = set(); rec_out = []
    for r in rec_all:
        if r not in seen:
            seen.add(r); rec_out.append(r)
    return {"hypotheses": hs, "recommendations": rec_out}
...
async def hypothesis_create(request: Request):
    ...
    # Prefer new schemas from Intent Agent if provided
    intent_v2: Dict[str, Any] = data.get("intent_v2") or {}
    intent_unique: Dict[str, Any] = data.get("intent_unique") or {}
    # Map unique→legacy intent shape when legacy is absent or minimal
    def _intent_from_unique(u: Dict[str, Any], v2: Dict[str, Any]) -> Dict[str, Any]:
        u = u or {}; v2 = v2 or {}
        # area/stage/domain
        stage = (u.get("domain") or v2.get("domain") or "catalysis").lower()
        area = stage  # if not specifically electro/thermal, downstream treats it generically
        # task: use research_question for more scientific framing
        task = u.get("research_question") or u.get("operation") or v2.get("intent_type") or ""
        # substrate/facet from system.surface like "Pt(111)"
        surf = ((u.get("system") or {}).get("surface") or (v2.get("target_system") or {}).get("surface") or "").strip()
        substrate, facet = None, None
        m = re.match(r"^\s*([A-Za-z0-9_-]+)\s*\(([^)]+)\)\s*$", surf)
        if m:
            substrate, facet = m.group(1), m.group(2).replace(" ", "")
        else:
            substrate = surf or None
        adsorbates = [str(x) for x in (u.get("adsorbates") or [])]
        return {
            "stage": stage,
            "area": area,
            "task": task,
            ...
            "conditions": {},
            "deliverables": {"target_products": (u.get("deliverables_expected") or [])},
            "tags": [],
        }
    if not intent or len(intent.keys()) <= 2:
        # replace only when legacy not provided
        intent = _intent_from_unique(intent_unique, intent_v2) if (intent_unique or intent_v2) else intent

    knowledge: Dict[str, Any] = data.get("knowledge") or {}
    # Optionals: prior results, literature hints
    if data.get("prior_results"): knowledge["prior_results"] = data.get("prior_results")
    if data.get("literature_hints"): knowledge["literature_hints"] = data.get("literature_hints")
    ...
    md = await _llm_markdown({"intent": intent, "knowledge": knowledge, "history": history, "intent_unique": intent_unique, "intent_v2": intent_v2})
    if not md:
    ...
    # 3) Structured hypotheses (unique schema output)
    structured_raw = await _llm_hypotheses_structured({
        "intent": intent,
        "intent_v2": intent_v2,
    ...
    }) or {"hypotheses": [
        {
            ...
            "rationale": ["Physical/chemical intuition based on the chosen system."],
            "predictions": ["Observable trends will follow adsorption site stability or DOS features."],
            "uncertainties": ["Exact site preference and barriers are unknown prior to calculation."],
            "recommendations": ["Run adsorption energies on key sites", "If needed, run NEB for key steps"],
        }
    ]}

    structured = _normalize_structured_bundle(structured_raw)

    # persist structured bundle & aggregated recs
    try:
        await _save_artifact(session_id, "hypotheses_structured", structured)
        await _save_artifact(session_id, "hypotheses_recommendations", structured.get("recommendations") or [])
    except Exception:
        pass

    return {
        "ok": True,
        "hypothesis": bundle.model_dump(),
        "graph": graph_fixed,
        "fields": fields,
        "hypotheses_structured": structured.get("hypotheses") or [],
        "recommendations_structured": structured.get("recommendations") or [],
    }
