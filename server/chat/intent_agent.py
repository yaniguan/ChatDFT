...
def _intent_system_prompt() -> str:
    ...
    The prompt text lives in ``server.chat.intent_prompt`` so that
    dependency-free consumers (the Modal training job, the teacher
    pipeline, the eval harness) can import it without pulling in the
    full FastAPI/sqlalchemy stack.

    The schema described in the prompt is enforced at runtime by
    ``server.chat.intent_schema.IntentSchema``. Any change to the prompt
    must be mirrored in that schema, and vice versa —
    ``tests/test_intent_schema.py`` tests the two together.
    """
