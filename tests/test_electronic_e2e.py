...
try:
    ...
    surf = post("/agent/structure/build_surface", {
        "element": "Zn", "surface_type": "0001",
        "nx": 4, "ny": 4, "nlayers": 3, "vacuum": 10.0, "fix_bottom": True
    })
    ...
    static = post("/agent/generate_script", {
        "calc_type": "static",
        "system": {"element": "Zn"},
    ...
    # Step 2: DOS script
    dos = post("/agent/generate_script", {
        "calc_type": "dos",
        "system": {"element": "Zn"},
        "params": {"kpoints": "12 12 1", "encut": 400}
    })
...
try:
    surf = post("/agent/structure/build_surface", {
        ...
        "nx": 4, "ny": 4, "nlayers": 4, "vacuum": 12.0, "fix_bottom": True
    })
    ...
    static = post("/agent/generate_script", {
        "calc_type": "static",
        "system": {"element": "Cu"},
        "params": {"kpoints": "8 8 1", "encut": 450}
    })
    check("Static SCF script", static.get("ok"))

    # Band script
    band = post("/agent/generate_script", {
        "calc_type": "band",
...
try:
    ...
    surf = post("/agent/structure/build_surface", {
        "element": "Cu", "surface_type": "111",
        "nx": 4, "ny": 4, "nlayers": 3, "vacuum": 15.0, "fix_bottom": True
    })
    check("Build Cu(111) surface", surf.get("ok"))
    ...
    # Build CO2 molecule
    mol = post("/agent/structure/build_molecule", {
        "smiles": "O=C=O", "label": "CO2", "cell_size": 20.0
    })
    ...
    configs = post("/agent/structure/generate_configs", {
        "poscar": poscar, "adsorbate": "CO2", "max_configs": 3
    })
    ...
    geo = post("/agent/generate_script", {
        "calc_type": "geo",
        "system": {"element": "Cu", "surface_type": "111"},
...
try:
    ...
    sites = post("/agent/structure/find_sites", {
        "poscar": poscar, "height": 2.0
    })
    ...
    configs = post("/agent/structure/generate_configs", {
        "poscar": poscar, "adsorbate": "H", "max_configs": 3
    })
    check("Generate 3×H configs", configs.get("ok"))
    n = len(configs.get("configs", []))
    check("3 H configs generated", n >= 3)
    for i, c in enumerate(configs.get("configs", [])[:3]):
        print(f"  → H config {i+1}: {c.get('label','')}  site={c.get('site_type','')}  n_atoms={c.get('n_atoms','')}")
...
try:
    ...
    mol = post("/agent/structure/build_molecule", {
        "smiles": "N[Cu](N)(N)N", "label": "Cu(NH3)4", "cell_size": 20.0
    })
    # fallback: plain NH3
    if not mol.get("ok"):
        mol = post("/agent/structure/build_molecule", {
            "smiles": "N", "label": "NH3", "cell_size": 20.0
        })
    ...
    # COHP script
    cohp = post("/agent/generate_script", {
        "calc_type": "cohp",
        "system": {"element": "Cu"},
        "params": {
...
try:
    ...
    configs = post("/agent/structure/generate_configs", {
        "poscar": poscar, "adsorbate": "CO", "max_configs": 2
    })
    ...
    # ELF script
    elf = post("/agent/generate_script", {
        "calc_type": "elf",
        "system": {"element": "Cu"},
        "params": {"kpoints": "8 8 1", "encut": 400}
    })
...
try:
    # Build clean Pt(111)
    surf_clean = post("/agent/structure/build_surface", {
        "element": "Pt", "surface_type": "111",
        "nx": 4, "ny": 4, "nlayers": 3, "vacuum": 15.0, "fix_bottom": True
    })
    ...
    # Place H on top site
    h_ads = post("/agent/structure/place_adsorbate", {
        "poscar": poscar_clean, "adsorbate": "H", "site_index": 0, "height": 1.8
    })
    ...
    bader_clean = post("/agent/generate_script", {
        "calc_type": "bader",
        "system": {"element": "Pt"},
        "params": {"kpoints": "8 8 1", "encut": 520}
    })
    ...
    bader_h = post("/agent/generate_script", {
        "calc_type": "bader",
        "system": {"element": "Pt"},
        "params": {"kpoints": "8 8 1", "encut": 520}
    })
...
try:
    ...
    surf = post("/agent/structure/build_surface", {
        "element": "Ni", "surface_type": "111",
        "nx": 4, "ny": 4, "nlayers": 4, "vacuum": 15.0, "fix_bottom": True
    })
    ...
    wf_clean = post("/agent/generate_script", {
    ...
    wf_co = post("/agent/generate_script", {
        "calc_type": "work_function",
        "system": {"element": "Ni"},
