...
try:
    ...
    n = r.get("n_atoms", 0)
    check("n_atoms = 17 (1 Cu + 4×N + 12×H = 17)", n == 17)
...
try:
    r = post(
        "/agent/structure/build_complex",
        {"metal": "Fe", "ligand": "CO", "n_coord": 6, "geometry": "octahedral", "bond_length": 1.84, "cell_size": 15.0},
    )
    check("Build Fe(CO)6", r.get("ok"))
    n = r.get("n_atoms", 0)
    check("n_atoms = 13 (1 Fe + 6×C + 6×O = 13)", n == 13)
...
try:
    r = post(
        "/agent/structure/build_complex",
        {"metal": "Au", "ligand": "Cl", "n_coord": 2, "geometry": "linear", "bond_length": 2.27, "cell_size": 12.0},
    )
...
try:
    r = post("/agent/structure/build_complex", {"metal": "Cu", "ligand": "UNKNOWN_LIGAND", "n_coord": 4})
    check("Returns ok=False for unknown ligand", not r.get("ok", True))
    check(
        "Error message mentions supported ligands",
        "support" in r.get("detail", "").lower()
        or "support" in r.get("error", "").lower()
...
surf_pt = post(
    ...
    {"element": "Pt", "surface_type": "111", "nx": 3, "ny": 3, "nlayers": 3, "vacuum": 10.0, "fix_bottom": True},
)
surf_cu = post(
    "/agent/structure/build_surface",
    {"element": "Cu", "surface_type": "111", "nx": 3, "ny": 3, "nlayers": 3, "vacuum": 10.0, "fix_bottom": True},
)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    try:
        ...
        print(
            f"  → Total={stats.get('total')} Pending={stats.get('pending')} Done={stats.get('done')} Failed={stats.get('failed')}"
        )
