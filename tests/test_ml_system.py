...
class TestEnvironmentSetup:
    ...
    def test_no_dead_imports_db_last(self):
        ...
        result = subprocess.run(
            ["grep", "-r", "from server.db" + "_last", "--include=*.py", "-l", "server/", "alembic/", "utils/"],
            capture_output=True,
            text=True,
            cwd=".",
        )
...
class TestDataIntegrity:
    ...
    def test_voronoi_features_shape(self):
        ...
        stg = SurfaceTopologyGraph(slab.get_positions(), slab.get_chemical_symbols(), slab.get_cell()[:])
        stg.build()
    ...
    def test_scf_features_shape(self):
        ...
        outcar = "\n".join(
            [f"DAV:   {i}   -100.{i:03d}   {1e-3 * np.exp(-0.2 * i):.3E}   {1e-2:.3E}" for i in range(30)]
        )
...
class TestPipelineIntegration:
    ...
    def test_rattled_structure_valid_for_voronoi(self):
        ...
        stg = SurfaceTopologyGraph(rattled.get_positions(), rattled.get_chemical_symbols(), rattled.get_cell()[:])
        stg.build()
        X = stg.node_feature_matrix()
...
class TestDeterminism:
    ...
    def test_einstein_rattle_deterministic(self):
        ...
        assert np.allclose(r1.get_positions(), r2.get_positions()), "Same seed should give same positions"
    ...
    def test_voronoi_graph_deterministic(self):
        ...
        slab = fcc111("Cu", size=(2, 2, 2), vacuum=10.0, a=3.615)
        stg1 = SurfaceTopologyGraph(slab.get_positions(), slab.get_chemical_symbols(), slab.get_cell()[:])
        stg1.build()
        stg2 = SurfaceTopologyGraph(slab.get_positions(), slab.get_chemical_symbols(), slab.get_cell()[:])
        stg2.build()
...
class TestSecurity:
    def test_cors_not_wildcard_with_credentials(self):
        ...
        assert not (has_wildcard and has_creds), "CRITICAL: CORS wildcard + credentials is a security vulnerability"

    def test_no_hardcoded_passwords(self):
        """No default passwords in database connection strings."""
        with open("server/db.py") as f:
            content = f.read()
        assert "password@" not in content.lower(), "Hardcoded password found in db.py"
