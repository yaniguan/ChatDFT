...
def main():
    ...
    # ─────────────────────────────────────────────────────────────────
    ax = ax_top
    ax.set_title("(a) Top-Down View: Topology Graph + Adsorption Sites", fontweight="bold", fontsize=11)
    ax.set_aspect("equal")
    ...
    for e in stg.edges:
        ...
        if dist < 8.0:  # generous cutoff for periodic cell
            ...
            ax.plot([p1[0], p2[0]], [p1[1], p2[1]], "-", color=C_EDGE, linewidth=lw, alpha=alpha, zorder=1)
    ...
    for site in sites:
        ...
        ax.scatter(sx, sy, s=100, c=color, marker=marker, edgecolors="black", linewidths=0.8, zorder=10, alpha=0.85)
    ...
    for stype in ["top", "bridge", "hollow_fcc", "hollow_hcp"]:
        ax.scatter(
            ...
            s=80,
            c=SITE_COLORS[stype],
            marker=SITE_MARKERS[stype],
            edgecolors="black",
            linewidths=0.8,
            label=SITE_LABELS[stype],
        )
    ...
    # ─────────────────────────────────────────────────────────────────
    ax = ax_side
    ax.set_title("(b) Side View: Layer Structure + Voronoi Bonds", fontweight="bold", fontsize=11)
    ...
    for e in stg.edges:
        ...
        if dist_xy < 5.0:
            ...
            ax.plot([p1[0], p2[0]], [p1[2], p2[2]], "-", color=C_EDGE, linewidth=lw, alpha=0.4, zorder=1)
    ...
    # ─────────────────────────────────────────────────────────────────
    ax = ax_zoom
    ax.set_title("(c) fcc vs hcp Hollow Site Distinction", fontweight="bold", fontsize=11)
    ax.set_aspect("equal")

    # Find one fcc and one hcp site for demonstration
    next((s for s in sites if s.site_type == "hollow_fcc"), None)
    next((s for s in sites if s.site_type == "hollow_hcp"), None)
    ...
    # Triangle edges + fill
    tri_poly = plt.Polygon(
        list(zip(tri_x, tri_y)), closed=True, facecolor=C_FCC, alpha=0.08, edgecolor=C_EDGE, linewidth=2.5, zorder=1
    )
    ax.add_patch(tri_poly)

    # Surface atoms (Layer 0)
    for tx, ty in zip(tri_x, tri_y):
        ax.scatter(tx, ty, s=500, c=C_ATOM_L0, edgecolors="white", linewidths=2, zorder=3)
        ax.text(tx, ty, "Cu", ha="center", va="center", fontsize=8, color="white", fontweight="bold", zorder=4)

    # fcc site marker at centroid
    cx_h = np.mean(tri_x)
    cy_h = np.mean(tri_y)
    ax.scatter(cx_h, cy_h, s=250, c=C_FCC, marker="^", edgecolors="black", linewidths=2, zorder=5)

    # Ghost circle where subsurface atom would be (but isn't)
    ax.scatter(cx_h, cy_h - 1.8, s=250, facecolors="none", edgecolors=C_FCC, linewidths=2, linestyle="--", zorder=3)
    ax.plot([cx_h, cx_h], [cy_h - 0.3, cy_h - 1.5], ":", color=C_FCC, linewidth=1.5, alpha=0.5)
    ax.text(
        cx_h,
        cy_h - 2.3,
        "No 2nd-layer\natom below",
    ...
    tri_poly2 = plt.Polygon(
        list(zip(tri_x2, tri_y2)), closed=True, facecolor=C_HCP, alpha=0.08, edgecolor=C_EDGE, linewidth=2.5, zorder=1
    )
    ...
    ax.scatter(cx_h2, cy_h2, s=250, c=C_HCP, marker="v", edgecolors="black", linewidths=2, zorder=5)

    # Subsurface atom (solid — present)
    ax.scatter(cx_h2, cy_h2 - 1.8, s=350, c=C_ATOM_L1, edgecolors="white", linewidths=2, zorder=3)
    ax.text(cx_h2, cy_h2 - 1.8, "Cu", ha="center", va="center", fontsize=7, color="white", fontweight="bold", zorder=4)
    ax.plot([cx_h2, cx_h2], [cy_h2 - 0.3, cy_h2 - 1.5], "-", color=C_HCP, linewidth=2, alpha=0.6)

    # Distance bracket
    ax.annotate(
        "",
        xy=(cx_h2 + 0.55, cy_h2 - 1.7),
        xytext=(cx_h2 + 0.55, cy_h2 - 0.15),
        arrowprops=dict(arrowstyle="<->", color="#555", lw=1.5),
    )
    ax.text(
        cx_h2 + 0.85, cy_h2 - 0.9, "d < 1.5 Å", fontsize=9, color="#555", rotation=90, va="center", fontweight="bold"
    )
    ...
    # ─────────────────────────────────────────────────────────────────
    ax = ax_voronoi
    ax.set_title("(d) Voronoi Tessellation — Surface Layer", fontweight="bold", fontsize=11)
    ax.set_aspect("equal")
    ...
    for ridge_verts in vor.ridge_vertices:
        if -1 not in ridge_verts:
            ...
            if xmin <= mx <= xmax and ymin <= my <= ymax:
                ax.plot([v0[0], v1[0]], [v0[1], v1[1]], "-", color="#B0BEC5", linewidth=1.0, alpha=0.7, zorder=1)
    ...
    # Draw surface atoms
    ax.scatter(surface_pos[:, 0], surface_pos[:, 1], s=200, c=C_ATOM_L0, edgecolors="white", linewidths=1.2, zorder=3)
    ...
    for site in sites:
        ...
        if xmin <= sx <= xmax and ymin <= sy <= ymax:
            marker = SITE_MARKERS[site.site_type]
            color = SITE_COLORS[site.site_type]
            ax.scatter(sx, sy, s=60, c=color, marker=marker, edgecolors="black", linewidths=0.6, zorder=5, alpha=0.8)
    ...
    # ─────────────────────────────────────────────────────────────────
    fig.suptitle("Voronoi Topology Graph Representation — Cu(111) 3×3 Slab", fontsize=15, fontweight="bold", y=1.01)
    fig.tight_layout()
