...
def fig_architecture():
    ...
    def draw_box(x, y, w, h, label, sublabel, layer, fontsize=11):
        box = FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.1",
                             facecolor=layer_colors[layer], edgecolor=border_colors[layer],
                             linewidth=1.5)
        ax.add_patch(box)
        ax.text(x + w/2, y + h/2 + 0.08, label, ha="center", va="center",
                fontsize=fontsize, fontweight="bold", color=border_colors[layer])
        if sublabel:
            ax.text(x + w/2, y + h/2 - 0.22, sublabel, ha="center", va="center",
                    fontsize=7, color="#555", style="italic")

    # Layer 1: UI
    draw_box(0.5, 6.5, 11, 1.2, "Streamlit UI — 9 Tabs",
             "Chat Pipeline | Structure Lab | Thermodynamics | QA & Debug | Knowledge Base | Analysis | HTP | Settings", "ui", 13)
    ...
    for y, label in [(7.1, "UI Layer"), (5.55, "API Layer"),
                      (4.05, "Agent Layer"), (2.55, "Science Layer"),
                      (1.05, "Infrastructure")]:
    ...
    for y in [6.45, 4.95, 3.45, 1.95]:
        ax.annotate("", xy=(6, y), xytext=(6, y + 0.1),
...
def fig_pipeline():
    ...
    ax.set_ylim(-0.5, 5)
    ax.axis("off")
    ax.set_title("End-to-End Pipeline: Natural Language → Free Energy Diagram",
    ...
    for i, (title, desc, bg, border) in enumerate(stages):
        ...
        ax.text(x + w/2, y - 0.25, f"Stage {i+1}", ha="center",
...
def fig_volcano_her():
    ...
    eta = [ex.expected_overpotential for ex in her_data]
    ...
    for i, (metal, dg, lj) in enumerate(zip(metals, dG_H, log_j0)):
        ...
        ax.annotate(label, (dg, lj), xytext=(dg + offset[0], lj + offset[1]),
...
def fig_scaling_oer():
    ...
    ax.plot(oh_range, scaling, "k--", linewidth=1.5, alpha=0.4,
            label="Scaling: ΔG$_{OOH*}$ = ΔG$_{OH*}$ + 3.2 eV")
...
def fig_free_energy_gallery():
    ...
    domain_colors = {
        "co2rr": "#1565C0", "her": "#2E7D32", "oer": "#E65100",
        "nrr": "#6A1B9A", "orr": "#C62828",
    }
    domain_titles = {
    ...
    fig.suptitle("Free Energy Diagrams — Golden Benchmark Dataset (U = 0 V$_{RHE}$)",
    ...
    for ax, (domain, examples) in zip(axes, GOLDEN_BY_DOMAIN.items()):
        ...
        for i in range(n - 1):
            ...
            ax.plot([i + 0.8, i + 1], [dG[i], dG[i + 1]], color=color,
        ...
        if n > 1:
            ...
            rds_val = diffs[rds]
            if rds_val > 0:
                ax.annotate(f"RDS\nΔG={rds_val:.2f}", xy=(rds + 0.5, (dG[rds] + dG[rds + 1]) / 2),
        ...
        ax.text(0.95, 0.05, f"η = {ex.expected_overpotential:.2f} V",
...
def fig_mechanism_graph():
    ...
    ax.set_ylim(-1, 7)
    ax.axis("off")
    ax.set_title("CO₂RR Mechanism on Cu(111): Reaction Network Graph",
    ...
    for src, dst, label in edges:
        ...
        x2, y2 = nodes[dst]
        dx, dy = x2 - x1, y2 - y1
        ax.annotate("", xy=(x2 - dx * 0.08, y2 - dy * 0.08),
                    xytext=(x1 + dx * 0.08, y1 + dy * 0.08),
                    arrowprops=dict(arrowstyle="-|>", color="#666",
                                    lw=1.5, connectionstyle="arc3,rad=0.1"))
        if label:
            mx, my = (x1 + x2) / 2, (y1 + y2) / 2
            ax.text(mx, my + 0.25, label, fontsize=7, color="#888",
                    ha="center", style="italic")

    # Draw nodes
    for name, (x, y) in nodes.items():
        nt = node_type(name)
        circle = plt.Circle((x, y), 0.6, facecolor=node_colors[nt],
                             edgecolor="#333", linewidth=1.5, zorder=5)
        ax.add_patch(circle)
        ax.text(x, y, name, ha="center", va="center", fontsize=9,
                fontweight="bold", zorder=6)

    # Legend
    for i, (label, color) in enumerate([("Gas phase", "#E3F2FD"),
                                         ("Adsorbed", "#FCE4EC"),
                                         ("Bare site", "#E8F5E9")]):
        ax.add_patch(plt.Circle((0.5, 0.5 - i * 0.6), 0.25, facecolor=color,
                                edgecolor="#333", linewidth=1))
        ax.text(1.0, 0.5 - i * 0.6, label, fontsize=9, va="center")

    # Pathway annotations
    ax.annotate("CO pathway\n(2e⁻)", xy=(6, 4), fontsize=10,
                color="#1565C0", fontweight="bold",
                bbox=dict(boxstyle="round", facecolor="#E3F2FD", alpha=0.8))
    ax.annotate("CH₄ pathway\n(8e⁻)", xy=(7.5, 0.2), fontsize=10,
...
def fig_voronoi_heatmap():
    ...
    ax.set_title("Voronoi Node Features Across Surfaces and Layers",
...
def fig_embedding_tsne():
    ...
    for ex in GOLDEN_SET:
        ...
        network_dict = {
            "reaction_network": [{"lhs": ex.expected_intermediates[:2],
                                   "rhs": ex.expected_intermediates[2:3]}],
    ...
    for domain in domain_colors:
        ...
        pts = text_2d[mask]
        ax.scatter(pts[:, 0], pts[:, 1], c=domain_colors[domain],
                   marker=domain_markers[domain], s=80, label=domain.upper(),
    ...
    fig.suptitle("Cross-Modal Embedding Space — 25 Reactions × 5 Domains",
...
def fig_radar():
    ...
    ax.plot(angles, chatdft, "o-", color="#2196F3", linewidth=2.5,
            markersize=8, label="ChatDFT (ours)")
    ax.fill(angles, baseline, color="#9E9E9E", alpha=0.1)
    ax.plot(angles, baseline, "s--", color="#9E9E9E", linewidth=2,
    ...
    ax.set_title("Algorithm Performance: ChatDFT vs Baselines",
...
def fig_scf_decision_tree():
    ...
    ax.set_ylim(0, 8)
    ax.axis("off")
    ax.set_title("SCF Convergence Diagnostic Decision Tree",
                 fontsize=14, fontweight="bold", pad=15)
...
def fig_bo_landscape():
    ...
    Z_true = np.array([[synthetic_energy_landscape(e, int(k), noise=0)
                         for e in encuts] for k in kppras])
    ...
    ax.scatter(obs_pts[:, 0], obs_pts[:, 1], c="red", s=60, edgecolors="white",
               linewidth=1.5, zorder=5, label="BO evaluations")
    ...
    ax.scatter(obs_pts[:, 0], obs_pts[:, 1], c="blue", s=60, edgecolors="white",
               linewidth=1.5, zorder=5)
    # Mark next suggestion
    next_e, next_k = opt.suggest_next()
    ax.scatter(next_e, next_k, c="lime", s=150, marker="*", edgecolors="black",
               linewidth=1.5, zorder=6, label=f"Next: ({next_e:.0f}, {next_k})")
    ax.set_xlabel("ENCUT (eV)")
    ax.set_ylabel("KPPRA")
...
def fig_golden_coverage():
    ...
    fig, ax = plt.subplots(figsize=(14, 5))
    ax.set_title("Golden Dataset Material Coverage — 25 Reactions Across 15 Materials",
                 fontsize=14, fontweight="bold")
    ...
    for mat, info in sorted(materials.items(), key=lambda x: -x[1]["count"]):
        ...
        for domain in domains:
            count = sum(1 for ex in GOLDEN_SET
                        if ex.expected_intent.get("system", {}).get("material") == mat
                        and ex.domain == domain)
            ax.bar(x, count, bar_width, bottom=bottom,
        ...
        if info["eta_min"] < 999:
            eta_str = f"{info['eta_min']:.2f}" if info["eta_min"] == info["eta_max"] \
                      else f"{info['eta_min']:.2f}-{info['eta_max']:.2f}"
            ax.text(x, bottom + 0.1, f"η={eta_str}V", ha="center",
                    fontsize=6, rotation=45, color="#555")
        x += 1
    ...
    legend_patches = [Patch(facecolor=c, label=d.upper())
...
def fig_results_table():
    ...
    ax.axis("off")
    ax.set_title("ChatDFT Benchmark Results Summary",
                 fontsize=16, fontweight="bold", pad=20)

    table_data = [
        ["Module", "Algorithm", "ChatDFT Result", "Baseline", "Improvement", "Metric"],
        ["Surface Rep.", "Voronoi topology graph", "4-class + symmetry", "3-class only",
         "33% more classes", "Site granularity"],
        ["Structure Gen.", "Einstein quantum rattle", "σ_H/σ_Pt = 13.9×", "σ = 0.1 Å fixed",
         "Mass-aware ZPE", "Physical correctness"],
        ["Hypothesis", "InfoNCE cross-modal", "AUC = 1.00", "AUC = 0.43",
         "+0.57 AUC", "Discrimination"],
        ["SCF Diagnostic", "FFT + sign-change", "100% accuracy", "50% accuracy",
         "+50% accuracy", "60 trajectories"],
        ["SCF Prediction", "Exponential OLS fit", "MAE = 3.6 steps", "MAE = 7.4 steps",
         "2.1× better", "30 trajectories"],
        ["Param. Search", "GP + EI (Bayesian)", "15 evaluations", "56 evaluations",
         "73% savings", "Same target error"],
        ["Golden Dataset", "Literature benchmark", "25 reactions", "3 reactions",
         "8.3× coverage", "5 domains, 25 DOIs"],
        ["Test Suite", "Unit + integration", "143 passing", "—",
         "100% pass rate", "5 test files"],
    ]
...
def fig_gnn_architectures():
    ...
    ax.set_ylim(0, 8)
    ax.axis("off")
    ax.set_title("GNN Architectures for Adsorption Energy Prediction",
                 fontsize=15, fontweight="bold", pad=20)

    # Shared input at top
    input_box = FancyBboxPatch((5.0, 7.0), 4.0, 0.8, boxstyle="round,pad=0.1",
                                facecolor="#E8E8E8", edgecolor="#333333", linewidth=2)
    ax.add_patch(input_box)
    ax.text(7.0, 7.4, "Voronoi Topology Graph\n(N,6) nodes + (2,2E) edges + (N,3) pos",
    ...
    for i, (name, desc, color, feature) in enumerate(models):
        x = 0.5 + i * 2.2
        # Model box
        box = FancyBboxPatch((x, 3.5), 2.0, 2.8, boxstyle="round,pad=0.15",
                              facecolor=color, edgecolor="white", linewidth=1.5, alpha=0.15)
        ax.add_patch(box)
        box_border = FancyBboxPatch((x, 3.5), 2.0, 2.8, boxstyle="round,pad=0.15",
                                     facecolor="none", edgecolor=color, linewidth=2)
        ax.add_patch(box_border)

        ax.text(x + 1.0, 6.0, name, ha="center", va="center",
                fontsize=11, fontweight="bold", color=color)
        ax.text(x + 1.0, 5.2, desc, ha="center", va="center",
                fontsize=7, color="#333")
        ax.text(x + 1.0, 4.2, feature, ha="center", va="center",
                fontsize=7, fontstyle="italic", color="#666",
                bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="#ddd"))

        # Arrow from input
        ax.annotate("", xy=(x + 1.0, 6.3), xytext=(7.0, 7.0),
                    arrowprops=dict(arrowstyle="->", color="#999", lw=1.2))

        # Arrow to output
        ax.annotate("", xy=(x + 1.0, 2.5), xytext=(x + 1.0, 3.5),
                    arrowprops=dict(arrowstyle="->", color="#999", lw=1.2))

    # Output box at bottom
    output_box = FancyBboxPatch((4.0, 1.5), 6.0, 0.8, boxstyle="round,pad=0.1",
                                 facecolor="#E8F5E9", edgecolor="#2E7D32", linewidth=2)
    ax.add_patch(output_box)
    ax.text(7.0, 1.9, "E_ads (eV) — Adsorption Energy Prediction",
            ha="center", va="center", fontsize=10, fontweight="bold", color="#2E7D32")

    # Legend for colour coding
    for color, label, y in [("#d62728", "No structure", 0.9),
                             ("#2ca02c", "Topology-based", 0.5),
                             ("#1f77b4", "Geometry-aware", 0.1)]:
        ax.add_patch(FancyBboxPatch((0.5, y), 0.3, 0.3, boxstyle="round,pad=0.05",
                                     facecolor=color, alpha=0.3, edgecolor=color))
        ax.text(1.0, y + 0.15, label, va="center", fontsize=8, color=color)

    # Complexity arrow at bottom
    ax.annotate("", xy=(13.0, 0.5), xytext=(1.0, 0.5),
                arrowprops=dict(arrowstyle="->", color="#666", lw=1.5))
    ax.text(7.0, 0.2, "Increasing geometric information →", ha="center",
...
def fig_gnn_message_passing():
    ...
    ax.annotate("m_ij", xy=(0.4, 0.25), fontsize=7, color="#2ca02c",
                fontweight="bold")
    ...
    arc = Arc((0, -0.3), 0.6, 0.6, angle=0, theta1=120, theta2=60,
              color="#ff7f0e", lw=2)
    ax.add_patch(arc)
    ...
    for i, j in edges:
        ax.plot([nodes[i][0], nodes[j][0]], [nodes[i][1], nodes[j][1]],
                color="#9467bd", alpha=0.3, lw=1.5)
    for pos in nodes:
        ax.add_patch(plt.Circle(pos, 0.2, color="#9467bd", alpha=0.4))
        # Vector feature arrows
        dx, dy = np.random.default_rng(42).normal(0, 0.3, 2)
        ax.annotate("", xy=(pos[0] + dx * 0.5, pos[1] + dy * 0.5), xytext=pos,
                    arrowprops=dict(arrowstyle="->", color="#9467bd", lw=1.5))
    ax.text(0, -1.4, "scalar + vector", ha="center", fontsize=7,
            fontstyle="italic", color="#9467bd")

    fig.suptitle("Information Flow: From Topology to Equivariance",
